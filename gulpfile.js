...
gulp.task('watch', function() {
  runSequence('build', [ 'watch-src', 'watch-vendor' ]);
});

gulp.task('build', function(callback) {
  runSequence('clean:dist', 'build-src', 'build-vendor', callback);
});

gulp.task('build-src', [ 'build-src:js', 'build-src:css', 'build-src:html' ]);
...
gulp.task('watch-vendor',
          function() { gulp.watch('bower.json'['build-vendor']); });

gulp.task('watch-src:js',
          function() { webpack(webpackConfig).watch({}, webpackLogger()); });

gulp.task('watch-src:css',
          function() { gulp.watch('./src/less/**/*', [ 'build-src:css' ]); });

gulp.task('watch-src:html',
          function() { gulp.watch('./src/html/**/*', [ 'build-src:html' ]); });
...
gulp.task('build-src:html', function() {
  return gulp.src('src/html/main.html').pipe(gulp.dest('dist'));
});
...
gulp.task('clean:dist', function() { return gulp.src('dist').pipe(clean()); });

gulp.task('build-electron', function(callback) {
  runSequence('build', 'build-electron:clean', 'build-electron:src', callback);
});

gulp.task('build-electron:clean',
          function() { return gulp.src('Electron.app').pipe(clean()); });
